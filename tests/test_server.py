...
class TestWeatherMCPServer:
    ...
    async def test_get_alerts_tool_impl(self):
        ...
        with patch(
            "weather_mcp.server.get_alerts_tool", new_callable=AsyncMock
        ) as mock_tool:
    ...
    async def test_get_forecast_tool_impl(self):
        ...
        with patch(
            "weather_mcp.server.get_forecast", new_callable=AsyncMock
        ) as mock_forecast:
    ...
    def test_main_block_logic(self):
        ...
        with (
            patch("weather_mcp.server.mcp.run") as mock_run,
